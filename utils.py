    ...
    for b in range(grid_table.shape[0]):
        ...
        for c in range(1, data_loader.num_classes):
            ...
            try:  # calculate prevalence/recall with decimal precision
                ...
                num_correct_recall += np.shape(np.intersect1d(labels_indexes, logits_indexes))[0] / \
                                      np.shape(labels_indexes)[0]
            ...
            try:  # calculate precision
                ...
                num_correct_precision += np.shape(np.intersect1d(labels_indexes, logits_indexes))[0] / \
                                         np.shape(logits_indexes)[0]
            except ZeroDivisionError:
                if np.shape(labels_indexes)[0] == 0:

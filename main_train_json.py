...
parser.add_argument('--doc_path', type=str, default='for_annotation')
parser.add_argument('--save_prefix', type=str, default='REALINVOICES', help='prefix for ckpt')
parser.add_argument('--test_path', type=str, default='corrected_annotations') # leave empty if no test data provided
...
parser.add_argument('--segment_grid', type=bool,
                    default=False)  # segment grid into two parts if grid is larger than cols_target
...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as sess:
        ...
        for iter in range(iter_start, params.iterations + 1):
            ...
            feeds = [network.data_grid, network.gt_classes, network.data_image, network.ps_1d_indices,
                     network.ghm_weights]
            ...
            if iter % params.log_disp_step == 0:
                ...
                print('LOSS CURVE: ' + ' >'.join(['{:d}:{:.3f}'.
                                                 format(i * params.log_disp_step, w) for i, w in
                                                  enumerate(loss_curve)]))
                print('TRAINING ACC CURVE: ' + ' >'.join(['{:d}:{:.3f}'.
                                                         format(i * params.log_disp_step, w) for i, w in
                                                          enumerate(training_acc_strict)]))
                print('TRAINING Statistic (Precision/Recall/Acc): %.3f / %.3f / %.3f (%.3f) | highest %.3f / %.3f / %.3f (%.3f)' \
                      % (precision, recall, acc_strict, acc_soft, max(training_precision),  max(training_recall), max(training_acc_strict),
            ...
            if iter % params.validation_step == 0 and len(data_loader.validation_docs):
                ...
                print('VALIDATION ACC (SOFT) CURVE: ' + ' >'.join(['{:d}:{:.3f}'.
                                                                  format(i * params.validation_step, w) for i, w in
                                                                   enumerate(validation_acc_soft)]))
                print('TRAINING RECALL CURVE: ' + ' >'.join(['{:d}:{:.2f}'.
                                                            format(i * params.log_disp_step, w) for i, w in
                                                             enumerate(training_recall)]))
                ...
                print('VALIDATION Statistic %d(%d) (Precision/Recall/Acc): %.3f / %.3f / %.3f (%.3f) | highest %.3f / %.3f / %.3f (%.3f) \n'
                      % (iter, idx * params.validation_step, precision, recall, acc_strict, acc_soft,
                         validation_precision[idx], validation_recall[idx], validation_acc_strict[idx], validation_acc_soft[idx]))
            ...
            if params.test_path != '' and iter % params.test_step == 0 and len(data_loader.test_docs):
                ...
                print('\n TEST Statistic (Precision/Recall/Acc): %.3f / %.3f / %.3f (%.3f) | highest %.3f / %.3f / %.3f (%.3f) \n'
                      % (precision,recall, acc_strict, acc_soft, test_precision[idx_precision], test_recall[idx_recall], test_acc_strict[idx_acc_strict], test_acc_soft[idx_acc_soft]))

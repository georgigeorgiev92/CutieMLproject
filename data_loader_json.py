...
class DataLoader():
    ...
    def __init__(self, params, update_dict=True, load_dictionary=False, data_split=0.75):
        ...
        #had to add DontCare class
        self.classes = ['DontCare', 'SENDER_NAME', 'SENDER_ADDRESS', 'RECEIVER_NAME', 'RECEIVER_ADDRESS', 'INVOICE_DATE', 'INVOICE_NUMBER',
                        'TOTAL_AMOUNT']
        ...
        self.rows_ulimit = params.rows_ulimit if hasattr(params,
                                                         'rows_ulimit') else 80  # handle OOM, must be multiple of self.encoding_factor
        self.cols_ulimit = params.cols_ulimit if hasattr(params,
                                                         'cols_ulimit') else 80  # handle OOM, must be multiple of self.encoding_factor
        ...
        self.test_docs, self.test_labels = self.load_data(self.test_doc_files,
                                                          update_dict=update_dict)  # TBD: optimize the update dict flag
    ...
    def next_batch(self):
        ...
        while True:
            ...
            rows, cols, pre_rows, pre_cols = self._cal_rows_cols(training_docs,
                                                                 extra_augmentation=self.data_augmentation_extra,
            ...
            if self.use_cutie2:
                images, ps_1d_indices = self._positional_sampling(self.doc_path, file_names, ps_indices_x, ps_indices_y,
                                                                  updated_cols)
        ...
        batch = {'grid_table': np.array(grid_table), 'gt_classes': np.array(gt_classes),
                 'data_image': np.array(images), 'ps_1d_indices': np.array(ps_1d_indices),
    ...
    def fetch_validation_data(self):
        ...
        while True:
            ...
            self.validation_data_tobe_fetched = list(
                set(self.validation_data_tobe_fetched).difference(set(selected_index)))
        ...
        def build_gt_pyramid(self, gt_classes):
            ...
            for gt_class in gt_classes:
                pad_v = np.pad(gt_class, ((same_padding_top, same_padding_bottom), (0, 0)), 'constant',
                               constant_values=((0, 0), (0, 0)))
                pad_h = np.pad(gt_class, ((0, 0), (same_padding_left, same_padding_right)), 'constant',
                               constant_values=((0, 0), (0, 0)))
    ...
    def fetch_test_data(self):
        ...
        while True:
            ...
            images, ps_1d_indices = [], []
            if self.use_cutie2:
                images, ps_1d_indices = self._positional_sampling(self.doc_test_path, file_names, ps_indices_x,
                                                                  ps_indices_y, updated_cols)
                if len(images) == batch_size:
                    break
            else:
                break
            ...
            for i, line in enumerate(content_dressed):
                ...
                if cnt[row] <= left_boundary:
                    ...
                    content_dressed_right.append([file_name, dressed_text, word_id, [x_left, y_top, x_right, y_bottom], \
                                                  ...
                                                  max(max(cnt_r.values()), self.cols_segment)])
                else:
                    cnt_r[row] += 1
                    content_dressed_right.append([file_name, dressed_text, word_id, [x_left, y_top, x_right, y_bottom], \
                                                  [left, top, right, bottom], max_rows,
...
# The _dress_text function uses the vocab.txt to look for known words and uses it for word piece tokenization. Refer BERT model.
    ...
    def grid_visualization(self, file_name, grid, label):
        ...
        cv2.putText(palette, file_name + "({},{})".format(height, width), (grid_box_h, grid_box_w), font, 0.6,
                    [255, 0, 0])
